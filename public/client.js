...
async function loadImageFromUri(uri) {
  const img = new Image();
  img.crossOrigin = 'Anonymous';
  return new Promise((resolve, reject) => {
    img.onload = async function() {
...
async function fetchItemsForImages() {
  const n = (window.location.search.indexOf('?n=') === 0)
    ...
    : 50;
  return await Promise.all(_.compact(_.range(0, n).map(async i => {
    try {
      const {canvas, imageKey} = await newClipartCanvas();
      const uri = canvas.toDataURL()
      return {uri, el: canvas};
    } catch(err) {
      // sometimes they error
...
async function projectWithUmap(items) {
  ...
  return umap.fitAsync(embeddings);
}
